 ...
 function enhance(){
   ...
   root.querySelectorAll('.fact-label').forEach(label=>{
     ...
     for(const c of countries){
       ...
       html=html.split(c.flag).join(`<span class="country-flag-tip" data-country="${safeName}" aria-label="${safeName}">${c.flag}</span>`);changed=true;
     }
     if(changed){
       ...
       value.querySelectorAll('.country-flag-tip').forEach(el=>{
         ...
         el.addEventListener('pointerdown',e=>{if(e.pointerType!=='mouse'){e.preventDefault();active===el?hide():show(el)}});
       });
     }
   });
 }
