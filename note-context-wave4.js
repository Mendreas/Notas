(() => {
  ...
  many('NOK',{
    ...
    500:['RS 14 Stavanger','A frente homenageia o serviço de salvamento marítimo com o histórico navio RS 14 Stavanger.','Mar, energia e navegação','O verso associa o mar norueguês à tecnologia, à energia e à segurança marítima.'],
    1000:['O mar aberto','A frente apresenta uma onda em alto-mar, síntese visual da relação da Noruega com o oceano.','Horizonte e ondas','O verso reduz o tema marítimo a um padrão abstrato inspirado na força do mar e do vento.']
  });

  many('SEK',{
    20:['Astrid Lindgren','Escritora sueca, criadora de Pippi das Meias Altas.','Småland','O verso evoca Småland, região ligada à infância e ao universo literário de Astrid Lindgren.'],
  ...
  const pln={10:['Mieszko I','primeiro governante histórico da Polónia'],20:['Bolesław I Chrobry','primeiro rei da Polónia'],50:['Casimiro III, o Grande','rei associado ao fortalecimento institucional do país'],100:['Władysław II Jagiełło','rei ligado à vitória de Grunwald'],200:['Sigismundo I, o Velho','monarca do Renascimento polaco'],500:['João III Sobieski','rei e comandante associado à vitória de Viena em 1683']};
  Object.entries(pln).forEach(([v,[p,d]])=>add('PLN',v,I(p,`A frente apresenta ${p}, ${d}.`,'A família atual organiza-se cronologicamente em torno de soberanos ligados a diferentes fases da formação do Estado.'),I('Símbolos do respetivo reinado','O verso reúne moedas, brasões, edifícios ou objetos associados à época do soberano representado.','Os motivos funcionam como síntese histórica do respetivo período.')));

  many('DKK',{
    50:['Ponte de Sallingsund','A frente apresenta a Ponte de Sallingsund.','Vaso de Skarpsalling','O verso apresenta o famoso recipiente neolítico de Skarpsalling.'],
    ...
    500:['Ponte Rainha Alexandrine','A frente apresenta a ponte que liga as ilhas da Zelândia e Møn.','Achado arqueológico de Keldby','O verso apresenta um objeto arqueológico de bronze encontrado em Keldby, na ilha de Møn.']
  });

  const czk={100:['Carlos IV','rei da Boémia e imperador do Sacro Império'],200:['Jan Amos Komenský','pedagogo e filósofo conhecido como Comenius'],500:['Božena Němcová','escritora central da literatura checa'],1000:['František Palacký','historiador e figura do renascimento nacional checo'],2000:['Ema Destinnová','soprano checa de carreira internacional'],5000:['Tomáš Garrigue Masaryk','primeiro presidente da Checoslováquia']};
  ...
  Object.entries(ron).forEach(([v,[p,d]])=>add('RON',v,I(p,`A frente apresenta ${p}, ${d} romeno de grande importância histórica ou cultural.`,'As notas romenas modernas são impressas em polímero.'),I('Património associado à personalidade','O verso apresenta edifícios, obras, instrumentos ou outros motivos diretamente relacionados com a figura da frente.')));
  ...
  Object.entries(ke).forEach(([v,t])=>add('KES',v,I('KICC · estátua de Jomo Kenyatta','A frente combina o Kenyatta International Convention Centre, em Nairobi, a estátua de Jomo Kenyatta e o Monte Quénia.','A nova geração foi concebida para representar identidade nacional e património.'),I(t[0].toUpperCase()+t.slice(1),`O verso desenvolve o tema da ${t}, através de cenas e infraestruturas representativas do Quénia moderno.`,'Cada denominação foi associada a um setor diferente da sociedade e da economia quenianas.')));

  const eg={5:['Mesquita de Ibn Tulun','Hapi, divindade do Nilo'],10:['Mesquita de Al-Rifa’i','motivos faraónicos'],20:['Mesquita de Muhammad Ali','carro de guerra e motivos faraónicos'],50:['Mesquita de Abu Huraiba','Templo de Edfu'],100:['Mesquita do Sultão Hassan','Grande Esfinge de Gizé'],200:['Mesquita de Qani-Bay','escriba sentado']};
  Object.entries(eg).forEach(([v,[f,b]])=>add('EGP',v,I(f,`A frente apresenta ${f}, exemplo do património arquitetónico islâmico do Cairo.`,'As notas egípcias combinam património islâmico na frente e motivos do Egito faraónico no verso.'),I(b,`O verso apresenta ${b}.`,'A composição liga a moeda contemporânea à longa continuidade histórica e arqueológica do Egito.')));
  ...
  Object.entries(hu).forEach(([v,[p,b]])=>add('HUF',v,I(p,`A frente apresenta ${p}, figura decisiva da história húngara.`,'A série atual manteve a tradição de associar cada denominação a uma personalidade histórica.'),I(b,`O verso apresenta ${b}, motivo relacionado com a época ou a vida da personalidade representada.`)));

  const is={500:['Jón Sigurðsson','independência islandesa'],1000:['Brynjólfur Sveinsson','património religioso e manuscritos'],2000:['Jóhannes S. Kjarval','arte e paisagem'],5000:['Ragnheiður Jónsdóttir','bordado e cultura doméstica'],10000:['Jónas Hallgrímsson','natureza, poesia e investigação científica']};
  Object.entries(is).forEach(([v,[p,b]])=>add('ISK',v,I(p,`A frente apresenta ${p}, personalidade marcante da história e cultura islandesas.`,'As notas islandesas valorizam figuras da independência, religião, arte, literatura e ciência.'),I('Motivos ligados à sua obra',`O verso reúne referências a ${b}.`,'Os elementos visuais ligam a personalidade à paisagem e à memória islandesas.')));
  ...
  Object.entries(al).forEach(([v,[p,d]])=>add('ALL',v,I(p,`A frente apresenta ${p}, ${d}.`,'A nova família combina retratos históricos com design contemporâneo.'),I('Património albanês associado','O verso apresenta arquitetura, símbolos, manuscritos ou lugares ligados à personalidade e ao período histórico representado.')));
  ...
  Object.entries(ua).forEach(([v,[p,b]])=>add('UAH',v,I(p,`A frente apresenta ${p}, uma das grandes figuras da literatura, ciência ou história ucranianas.`,'A atual família combina retratos com elementos de segurança modernos.'),I(b,`O verso apresenta ${b}, local ou instituição associado ao legado da personalidade.`)));
  ...
  Object.entries(ru).forEach(([v,city])=>add('RUB',v,I(city,`A frente apresenta monumentos e arquitetura associados a ${city}.`,'A circulação russa combina desenhos de diferentes gerações; as notas mais recentes adotam uma organização por regiões e cidades de referência.'),I('Paisagem e património regional',`O verso apresenta monumentos, infraestruturas ou paisagens ligados a ${city} e à respetiva região.`,'O motivo concreto depende da geração da nota representada na imagem.')));
  ...
  Object.entries(am).forEach(([v,p])=>add('AMD',v,I(p,`A frente homenageia ${p}, personalidade ligada à literatura, música, arte, xadrez ou história religiosa arménia.`,'A terceira geração utiliza substrato compósito e forte integração de elementos históricos e culturais.'),I('Obra e património arménios','O verso desenvolve referências à obra, ao contexto histórico ou aos lugares associados à personalidade representada.')));
