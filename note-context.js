(()=>{
 ...
 function infoFor(currency,value,side){
   const k=`${currency}:${Number(value)}`;
   for(const s of sources()){const i=s?.[k]?.[side];if(i)return i;}
   const g=window.NOTE_CONTEXT_GLOBAL;
   if(g?.has?.(currency,value)){const i=g.get?.(currency,value,side);if(i)return i;}
   if(g?.get){const i=g.get(currency,value,side);if(i)return i;}
   return null;
 }
 function render(src){
   ...
   const side=n.front===src?'front':'back',i=infoFor(n.currency,n.value,side); if(!i)return;
   let box=document.querySelector('#imageViewerContext');
   if(!box){box=document.createElement('div');box.id='imageViewerContext';box.className='image-viewer-context';viewer.querySelector('.image-viewer-shell')?.appendChild(box);}
 ...
 function hook(){
   const old=window.openImageViewer;if(typeof old!=='function'||old.__contextHook)return;
   function wrapped(src,label='Nota'){old(src,label);requestAnimationFrame(()=>render(src));}
   wrapped.__contextHook=true;window.openImageViewer=wrapped;
 }
 hook();
